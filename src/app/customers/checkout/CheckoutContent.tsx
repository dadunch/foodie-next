    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-32">
            ...
            <main className="max-w-7xl mx-auto px-4 py-6">
                <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 content-center">
                        <label className={`relative flex flex-col items-center p-6 border-2 rounded-xl cursor-pointer transition-all ${paymentMethod === 'CASH' ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-green-300'}`}>
                            <input type="radio" name="payment-method" value="CASH" checked={paymentMethod === 'CASH'} onChange={(e) => setPaymentMethod(e.target.value)} className="sr-only" />
                        ...
                        <label className={`relative flex flex-col items-center p-6 border-2 rounded-xl cursor-pointer transition-all ${paymentMethod === 'Qris' ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-green-300'}`}>
                            <input type="radio" name="payment-method" value="Qris" checked={paymentMethod === 'Qris'} onChange={(e) => setPaymentMethod(e.target.value)} className="sr-only" />
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div className="space-y-3 mb-4">
                        <div className="flex justify-between items-center text-gray-700">
                            ...
                            <button onClick={() => setShowCartModal(true)} className="text-green-500 hover:text-green-600 font-medium text-sm">
                                Lihat Keranjang
                            </button>
                        </div>
                    </div>
            ...
            <div className="fixed bottom-20 left-0 right-0 bg-white border-t shadow-lg p-4 z-20">
                <button onClick={() => setShowConfirmModal(true)} className="w-full bg-green-500 text-white py-4 rounded-lg font-semibold text-lg hover:bg-green-600 transition-colors">
                    Bayar Sekarang
                </button>
            </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl max-w-md w-full p-8 shadow-2xl text-center">
                        ...
                        <div className="space-y-3">
                            <button onClick={() => { setShowQRModal(false); router.push(`/customers/history?table=${table}`); }} className="w-full bg-green-500 text-white py-3 rounded-lg font-semibold hover:bg-green-600 transition-colors">
                                Selesai
                            </button>
                            <button onClick={() => setShowQRModal(false)} className="w-full bg-gray-200 text-gray-700 py-3 rounded-lg font-semibold hover:bg-gray-300 transition-colors">
                                Batal
                            </button>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden shadow-2xl flex flex-col">
                                ...
                                {checkoutData.map((item) => (
                                    <div key={item.id} className="bg-gray-50 rounded-lg p-4 flex gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            {item.catatan && <p className="text-xs text-gray-500 italic mb-2">Catatan: {item.catatan}</p>}
                        ...
                        <div className="p-6 border-t bg-gray-50">
                            <button onClick={() => setShowCartModal(false)} className="w-full bg-green-500 text-white py-3 rounded-lg font-semibold hover:bg-green-600 transition-colors">
                                Tutup
                            </button>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 backdrop-blur-md bg-white/30 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setShowConfirmModal(false)} className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-lg font-semibold hover:bg-gray-300 transition-colors">
                                Tinjau Pesanan
                            </button>
                            <button onClick={handlePayment} className="flex-1 bg-green-500 text-white py-3 rounded-lg font-semibold hover:bg-green-600 transition-colors">
                                Bayar Sekarang
                            </button>
                        </div>
                    </div>
